	...
	/*Mostrará una ventana modal con un mensaje cuando showModal = true.
	  Las ventanas modales se mostrarán cuando el usuario inicia sesión, 
	  actualiza sus datos (en play.php) o hay un error de conexión (en index.php)*/	
	 function showModalWindow(){
	 	
	 	if (showModal){
	 		
	 		$('#windowMessage').modal('show');
	 		showModal = false;
	 	}
	 }
	...
	/*Función que hace que se muestre el botón math dice cuando haya una operación disponible y el
	borde de la pantalla de operaciones cuando hayamos pulsado un dado.*/
	function mostrarBotonMath(){
		//Mostramos el borde de la pantalla operaciones si hemos pulsado un dado
		if(numDadosPulsados === 1){
			//Capturamos el contenedor de la pantalla de operaciones para hacerlo visible.
			var pantallaOp = document.getElementById("pantalla");
			pantallaOp.style.border = 'solid';
		}
		//Mostramos el botón mathdice si hemos pulsado dos dados
		if(numDadosPulsados === 2){
			//Capturamos el botón math dice de resultados para hacerlo visible.
